...
/**
 * Unit tests for {@link ExchangeTestResult}.
 *
 * @author Rossen Stoyanchev
 */
public class ExchangeTestResultTests {
	...
	void status() {
		...
		assertThat(ExchangeTestResult.of(spec)).hasStatusOk().hasStatus2xxSuccessful();
	}
	...
	void contentType() {
		...
		ExchangeTestResult result = ExchangeTestResult.of(spec);
		assertThat(result).hasStatusOk();
		assertThat(result).contentType().isEqualTo("text/plain;charset=ISO-8859-1");
		assertThat(result).hasContentTypeCompatibleWith(MediaType.TEXT_PLAIN);
	}
	...
	void cookies() {
		...
		ExchangeTestResult result = ExchangeTestResult.of(spec);
		assertThat(result).hasStatusOk();
		assertThat(result).cookies().hasValue("foo", "bar");
	...
	void bodyText() {
		...
		ExchangeTestResult result = ExchangeTestResult.of(spec);
		assertThat(result).hasStatusOk();
		assertThat(result).contentType().isCompatibleWith(MediaType.TEXT_PLAIN);
		assertThat(result).bodyText().isEqualTo("Hello World");
		assertThat(result).hasBodyTextEqualTo("Hello World");
	}
